import type { State } from '@tyck/core'
import {
  ConfirmPrompt,
...
export function groupMultiselect<Options extends Option<Value>[], Value>(opts: GroupMultiSelectOptions<Options, Value>) {
  const opt = (
    option: Option<Value>,
    state:
    | 'inactive'
 ...
 */
export async function group<T>(prompts: PromptGroup<T>,
  opts?: PromptGroupOptions<T>): Promise<Prettify<PromptGroupAwaitedReturn<T>>> {
